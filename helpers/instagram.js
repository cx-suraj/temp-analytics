...
const self = {
	get: (path, sessionId, tryParse = true, params) =>
								...
								case 302: {
									switch (res.headers.location) {
										case insta + 'accounts/login/':
											return reject(429);
										case insta +
											'accounts/login/?next=/accounts/edit/%3F__a%3D1':
											return reject(401);
										default: {
											if (
												res.headers.location.startsWith(
													insta + 'challenge/?next='
												)
											)
												return reject(409);
							...
							try {
								...
								try {
									resolve(
										Object.values(
											Object.values(
												JSON.parse(body.match(/_sharedData = (.+);/)[1])[
													'entry_data'
												]
											)[0][0]['graphql']
										)[0]
	...
	search: (query, sessionId) =>
		new Promise((resolve, reject) =>
			self
				.get('web/search/topsearch', sessionId, false, {
					context: 'blended',
					query,
				})
	...
	graphQL: (query, queryHash, sessionId) =>
		self.get('graphql/query', sessionId, undefined, {
	...
	partialPost: (post) => ({
		...
		location: post['node']['location']
			? {
					id: post['node']['location']['id'],
					name: post['node']['location']['name'],
					...(post['node']['location']['address_json']
						? {
								city: JSON.parse(post['node']['location']['address_json'])[
									'city_name'
								],
						  }
						: {}),
		...
		views: post['node']['video_view_count']
			...
			: post['node']['edge_media_preview_like']['count'] || null,
		link: insta + 'p/' + post['node']['shortcode'],
	}),
	...
	fullPost: (post) => {
		...
		return {
			...
			location: post['location']
				? {
						...
						...(post['location']['address_json']
							? {
									city: JSON.parse(post['location']['address_json'])[
										'city_name'
									],
							  }
							: {}),
			...
			...(post['__typename'] === 'GraphSidecar'
				? {
						contents: post['edge_sidecar_to_children']['edges'].map(
							(content) => ({
								type: content['node']['is_video'] ? 'video' : 'photo',
								url: content['node']['is_video']
									? content['node']['video_url']
									: content['node']['display_url'],
								...(content['node']['is_video']
									? {
											thumbnail: content['node']['display_url'],
											views: content['node']['video_play_count'],
									  }
									: {}),
							})
						),
			...
			...(post['edge_media_to_tagged_user']
				? {
						tagged: post['edge_media_to_tagged_user']['edges'].map(
							(u) => u['node']['user']['username']
						),
				  }
				: {}),
			...
			...(post['edge_media_preview_comment']
				? {
						comments: post['comments_disabled']
							? null
							: post['edge_media_preview_comment']['edges'].map(
									self.postComment
							  ),
...
module.exports = class Insta {
	...
	getAccountNotifications() {
		return new Promise((resolve, reject) => {
			...
			self
				.get('accounts/activity', this.sessionId)
				.then((res) => {
					resolve(
						res['activity_feed']['edge_web_activity_feed']['edges']
							.map((item) => item['node'])
							.map((notification) => ({
								id: notification['id'],
	...
	getAccountStories() {
		return new Promise((resolve, reject) => {
			if (!this.sessionId) return reject(401);
			self
				.get('', this.sessionId, false, { __a: undefined })
				.then((body) => {
					self
						.graphQL(
							undefined,
							body.match(
								/<link rel="preload" href="\/graphql\/query\/\?query_hash=(.+)&amp;/
							)[1],
							this.sessionId
						)
						.then((body) => {
							resolve(
								body['user']['feed_reels_tray']['edge_reels_tray_to_reel'][
									'edges'
								].map((item) => ({
									unread:
										item['node']['latest_reel_media'] !== item['node']['seen'],
									author: {
	...
	getProfile(username = this.username, anonymous = false) {
		return new Promise((resolve, reject) =>
			self
				.get(username, anonymous ? null : this.sessionId)
				.then((profile) => {
					const id = profile['id'],
						access =
							!profile['is_private'] ||
					...
					resolve({
						...
						lastPosts: access
							? profile['edge_owner_to_timeline_media']['edges'].map((post) =>
									self.partialPost(post)
							  )
							: null,
						...
						...(this.sessionId
							? {
									user: {
										mutualFollowers: profile['edge_mutual_followed_by'][
											'edges'
										].map((item) => item['node']['username']),
	...
	async _getQueryHashs() {
		...
		const { Consumer, TagPageContainer, LocationPageContainer } =
				Object.fromEntries(
					[
						...(
			...
			locationScriptBody = await self.get(
						...
						.then((data) =>
							resolve(
								data['reels_media'][0]
									? {
											unread:
												data['reels_media'][0]['latest_reel_media'] !==
												data['reels_media'][0]['seen'],
											author: {
											...
											user: {
												requesting:
													data['reels_media'][0]['user']['requested_by_viewer'],
												following:
													data['reels_media'][0]['user']['followed_by_viewer'],
											},
	...
	getProfileStory(username = this.username) {
		return new Promise((resolve, reject) => {
			this._getProfileId(username)
				.then((id) => this.getProfileStoryById(id).then(resolve).catch(reject))
				.catch(reject);
		});
	}
	async getProfilePostsById(profileId, maxCount, pageId) {
		const res = await self.graphQL(
			...
			(
				await this._getQueryHashs()
			).post,
			this.sessionId
		);
		return Object.assign(
			res['user']['edge_owner_to_timeline_media']['edges'].map((item) =>
				self.fullPost(item['node'])
			),
			{
				nextPageId: res['user']['edge_owner_to_timeline_media']['page_info'][
					'has_next_page'
				]
					? res['user']['edge_owner_to_timeline_media']['page_info'][
							'end_cursor'
					  ]
					: undefined,
			}
	...
	async getProfilePosts(profileUsername, maxCount, pageId) {
		return this.getProfilePostsById(
			await this._getProfileId(profileUsername),
			maxCount,
			pageId
		);
	}
	getHashtag(hashtag) {
		return new Promise((resolve, reject) => {
			...
			self
				.get(path, this.sessionId)
				.then((hashtag) =>
					resolve({
						pic: hashtag['profile_pic_url'],
						posts: hashtag['edge_hashtag_to_media']['count'],
						featuredPosts: hashtag['edge_hashtag_to_top_posts']['edges'].map(
							(post) => self.partialPost(post)
						),
						lastPosts: hashtag['edge_hashtag_to_media']['edges'].map((post) =>
							self.partialPost(post)
						),
						link: insta + path,
						...(this.sessionId
	...
	async getHashtagPosts(hashtag, maxCount, pageId) {
		...
		return Object.assign(
			res['hashtag']['edge_hashtag_to_media']['edges'].map((post) =>
				self.partialPost(post)
			),
			{
				nextPageId: res['hashtag']['edge_hashtag_to_media']['page_info'][
					'has_next_page'
				]
			...
			self
				...
				.then((location) => {
					...
					resolve({
						...
						featuredPosts: location['edge_location_to_top_posts']['edges'].map(
							(post) => self.partialPost(post)
						),
						lastPosts: location['edge_location_to_media']['edges'].map((post) =>
							self.partialPost(post)
						),
						link: insta + path,
					});
	...
	async getLocationPostsById(locationId, maxCount, pageId) {
		...
		return Object.assign(
			res['location']['edge_location_to_media']['edges'].map((post) =>
				self.partialPost(post)
			),
			{
				nextPageId: res['location']['edge_location_to_media']['page_info'][
					'has_next_page'
				]
	...
	getPost(shortcode) {
		return new Promise((resolve, reject) => {
			self
				.get(`p/${shortcode}`, this.sessionId)
	...
	async getPostComments(shortcode, maxCount, pageId) {
		...
		return Object.assign(
			res['shortcode_media']['edge_media_to_parent_comment']['edges'].map(
				self.postComment
			),
			{
				nextPageId: res['shortcode_media']['edge_media_to_parent_comment'][
					'page_info'
				]['has_next_page']
					? res['shortcode_media']['edge_media_to_parent_comment']['page_info'][
							'end_cursor'
					  ]
					: undefined,
			}
	...
	subscribeAccountNotifications(
		callback,
		{ interval = 30, lastNotificationId }
	) {
		let active = true;
		const checkNewNotifications = () => {
			...
			(async () => {
				try {
					...
					const lastNotificationIndex = notifications.findIndex(
	...
	subscribeUserPosts(
		username,
		callback,
		{ interval = 30, lastPostShortcode, fullPosts = false } = {}
	) {
		let active = true;
		const checkNewPosts = () => {
			if (!active) return;
			(async () => {
				try {
					...
					const lastPostIndex = profile.lastPosts.findIndex(
						(post) => post.shortcode === lastPostShortcode
					);
					if (lastPostIndex !== -1) {
						for (let i = lastPostIndex - 1; i > -1; i--) {
							callback(
								fullPosts
									? await this.getPost(profile.lastPosts[i].shortcode)
									: profile.lastPosts[i]
							);
						}
					}
	...
	subscribeHashtagPosts(
		hashtagName,
		callback,
		{ interval = 30, lastPostShortcode = undefined, fullPosts = false } = {}
	) {
		let active = true;
		const checkNewPosts = () => {
			if (!active) return;
			(async () => {
				try {
					...
					const lastPostIndex = hashtag.lastPosts.findIndex(
						(post) => post.shortcode === lastPostShortcode
					);
					for (let i = lastPostIndex - 1; i > -1; i--) {
						callback(
							fullPosts
								? await this.getPost(hashtag.lastPosts[i].shortcode)
								: hashtag.lastPosts[i]
						);
					}
